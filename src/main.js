...
function analyzeSalesData(data, options) {
    ...
    if (!calculateRevenue || typeof calculateRevenue !== 'function') {
            throw new Error('calculateRevenue не определено');
        }
        if (!calculateBonus || typeof calculateBonus !== 'function') {
            throw new Error('calculateBonus не определено');
        }
    ...
    data.purchase_records.forEach(record => { // Чек 
        ...
        record.items.forEach(item => {
            ...
            let revenue = calculateRevenue(item, product); // Посчитать выручку (revenue) с учётом скидки через функцию calculateRevenue
